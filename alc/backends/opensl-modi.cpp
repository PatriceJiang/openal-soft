...
struct OpenSLPlayback final : public BackendBase {
    ...
    void process(SLBufferQueueItf bq) noexcept;
    static void processC(SLBufferQueueItf bq, void *context) noexcept
...
void OpenSLPlayback::process(SLBufferQueueItf) noexcept
{
...
void OpenSLPlayback::stop()
{
    ...
    SLBufferQueueItf bufferQueue;
    result = VCALL(mBufferQueueObj,GetInterface)(SL_IID_BUFFERQUEUE, &bufferQueue);
    PRINTERR(result, "bufferQueue->GetInterface");
    if(SL_RESULT_SUCCESS == result)
    {
...
void OpenSLCapture::process(SLBufferQueueItf) noexcept
{
...
// #ifdef SL_ANDROID_DATAFORMAT_PCM_EX
    ...
    if(SL_RESULT_SUCCESS == result)
    {
        result = VCALL(mRecordObj,GetInterface)(SL_IID_BUFFERQUEUE, &bufferQueue);
...
void OpenSLCapture::captureSamples(al::byte *buffer, uint samples)
{
    ...
    {
        const SLresult result{VCALL(mRecordObj,GetInterface)(SL_IID_BUFFERQUEUE,
            &bufferQueue)};
        PRINTERR(result, "recordObj->GetInterface");
